...
static MEMWRAP *MakeMemWrap(void *tz, int type, int size)
{
	MEMWRAP *tmp;
...
FCEUFILE * FCEU_fopen(const char *path, const char *ipsfn, char *mode, char *ext)
{
	...
	size = 0;
	data = FCEUD_LoadFromDatabase(path, &size);

	if(data && size)
	{
		fceufp->fp=MakeMemWrap(data, 2, size);
		fceufp->type=2;
		if(ipsfile)
		{
			fceufp->type=3;
	...
	if((t=FCEUD_UTF8fopen(path,mode)))
	{
		...
		if(ipsfile)
		{
			if(!(fceufp->fp=MakeMemWrap(t, 0, 0)))
			{
				free(fceufp);
				return(0);
			}
