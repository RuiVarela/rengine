...
static FCU_INLINE uint8 RdMemNorm(unsigned int A)
{
 return(_DB=ARead[A](A));
}

static FCU_INLINE void WrMemNorm(unsigned int A, uint8 V)
{
 BWrite[A](A,V);
}
...
static FCU_INLINE uint8 RdMemHook(unsigned int A)
{
 if(X.ReadHook)
...
static FCU_INLINE void WrMemHook(unsigned int A, uint8 V)
{
 if(X.WriteHook)
  X.WriteHook(&X,A,V);
 else
...
static FCU_INLINE uint8 RdRAMFast(unsigned int A)
{
 return(_DB=RAM[A]);
}

static FCU_INLINE void WrRAMFast(unsigned int A, uint8 V)
{
 RAM[A]=V;
}
