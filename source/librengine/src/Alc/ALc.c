...
#include "../OpenAL32/alMain.h"
#include "../OpenAL32/alSource.h"
#include "AL/al.h"
#include "AL/alc.h"
#include "../OpenAL32/alThunk.h"
#include "../OpenAL32/alSource.h"
#include "../OpenAL32/alBuffer.h"
#include "../OpenAL32/alAuxEffectSlot.h"
#include "../OpenAL32/alDatabuffer.h"
#include "../OpenAL32/bs2b.h"
#include "../OpenAL32/alu.h"
...
static void ProbeDeviceList()
{
    ...
    rg_free(alcDeviceList); alcDeviceList = NULL;
    alcDeviceListSize = 0;
...
static void ProbeAllDeviceList()
{
    ...
    rg_free(alcAllDeviceList); alcAllDeviceList = NULL;
...
static void ProbeCaptureDeviceList()
{
    ...
    rg_free(alcCaptureDeviceList); alcCaptureDeviceList = NULL;
...
{                                                                            \
    ...
    if(len == 0)                                                             \
        return;                                                              \
                                                                             \
    temp = rg_realloc(alc##type##List, alc##type##ListSize + len + 2);          \
    if(!temp)                                                                \
    {                                                                        \
        AL_PRINT("rg_realloc failed to add %s!\n", name);                       \
        return;                                                              \
    }                                                                        \
    alc##type##List = temp;                                                  \
...
ALC_API ALCdevice* ALC_APIENTRY alcCaptureOpenDevice(const ALCchar *deviceName, ALCuint frequency, ALCenum format, ALCsizei SampleSize)
{
    ...
    device = rg_calloc(1, sizeof(ALCdevice));
    if(!device)
    {
...
ALC_API const ALCchar* ALC_APIENTRY alcGetString(ALCdevice *pDevice,ALCenum param)
{
    ...
    case ALC_DEFAULT_DEVICE_SPECIFIER:
        ...
        rg_free(alcDefaultDeviceSpecifier);
        alcDefaultDeviceSpecifier = rg_strdup(alcDeviceList ? alcDeviceList : "");
    ...
    case ALC_DEFAULT_ALL_DEVICES_SPECIFIER:
        ...
        rg_free(alcDefaultAllDeviceSpecifier);
        alcDefaultAllDeviceSpecifier = rg_strdup(alcAllDeviceList ?
                                              alcAllDeviceList : "");
    ...
    case ALC_CAPTURE_DEFAULT_DEVICE_SPECIFIER:
        ...
        rg_free(alcCaptureDefaultDeviceSpecifier);
        alcCaptureDefaultDeviceSpecifier = rg_strdup(alcCaptureDeviceList ? alcCaptureDeviceList : "");
...
ALC_API ALCcontext* ALC_APIENTRY alcCreateContext(ALCdevice *device, const ALCint *attrList)
{
    ...
    if(device->Bs2bLevel > 0 && device->Bs2bLevel <= 6)
    {
        if(!device->Bs2b)
        {
            device->Bs2b = rg_calloc(1, sizeof(*device->Bs2b));
    ...
    temp = rg_realloc(device->Contexts, (device->NumContexts+1) * sizeof(*device->Contexts));
    if(!temp)
    {
    ...
    ALContext = rg_calloc(1, sizeof(ALCcontext));
    if(!ALContext)
    {
        alcSetError(device, ALC_OUT_OF_MEMORY);
        ProcessContext(NULL);
...
ALC_API ALCboolean ALC_APIENTRY alcCloseDevice(ALCdevice *pDevice)
{
    ...
    *list = (*list)->next;
    ...
    rg_free(pDevice->Bs2b);
    pDevice->Bs2b = NULL;

    rg_free(pDevice->szDeviceName);
    pDevice->szDeviceName = NULL;

    rg_free(pDevice->Contexts);
