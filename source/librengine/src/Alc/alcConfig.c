...
//                    AL_PRINT("found block '%s'\n", nextBlock->name);
            ...
            if(!nextBlock)
            {
                nextBlock = rg_realloc(cfgBlocks, (cfgCount+1)*sizeof(ConfigBlock));
                if(!nextBlock)
                {
                ...
                nextBlock->name = rg_strdup(buffer+1);
...
//                AL_PRINT("found new block '%s'\n", nextBlock->name);
        ...
        if((size_t)(ent-curBlock->entries) >= curBlock->entryCount)
        {
            ...
            ent = rg_realloc(curBlock->entries, (curBlock->entryCount+1)*sizeof(ConfigEntry));
            if(!ent)
            {
...
void ReadALConfig(void)
{
    ...
    cfgBlocks = rg_calloc(1, sizeof(ConfigBlock));
    cfgBlocks->name = rg_strdup("general");
    cfgCount = 1;
...
void FreeALConfig(void)
{
    ...
    for(i = 0;i < cfgCount;i++)
    {
        ...
        {
           rg_free(cfgBlocks[i].entries[j].key);
           rg_free(cfgBlocks[i].entries[j].value);
        }
        rg_free(cfgBlocks[i].entries);
        rg_free(cfgBlocks[i].name);
    }
    rg_free(cfgBlocks);
    cfgBlocks = NULL;
    cfgCount = 0;
