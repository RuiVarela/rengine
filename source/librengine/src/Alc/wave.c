...
static ALCboolean wave_open_playback(ALCdevice *device, const ALCchar *deviceName)
{
    ...
    fname = GetConfigValue("wave", "file", "dump.wav");
    if(!fname[0])
        return ALC_FALSE;
    ...
    data = (wave_data*)rg_calloc(1, sizeof(wave_data));
    ...
    device->szDeviceName = rg_strdup(deviceName);
...
static ALCboolean wave_reset_playback(ALCdevice *device)
{
    ...
    data->buffer = rg_malloc(data->size);
    if(!data->buffer)
    {
        AL_PRINT("buffer rg_malloc failed\n");
        return ALC_FALSE;
    }
