...
#include "../OpenAL32/alEffect.h"
#include "../OpenAL32/alError.h"
...
static ALboolean AllocLines(ALboolean eaxFlag, ALuint frequency, ALverbState *State)
{
    ...
    {
        newBuffer = rg_realloc(State->SampleBuffer, sizeof(ALfloat) * totalSamples);
        if(newBuffer == NULL)
            return AL_FALSE;
...
ALeffectState *VerbCreate(void)
{
    ...
    State = rg_malloc(sizeof(ALverbState));
    if(!State)
        return NULL;
