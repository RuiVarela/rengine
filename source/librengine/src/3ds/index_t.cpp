...
int indexCheckAlloc(index_t* index)
{
   ...
   if ((index->allocCount == 0) && (index->count == 0))
   {
      ...
      index->indices = (int*)rg_malloc(index->allocCount * sizeof(int));
   ...
   if (index->count >= index->allocCount)
   {
      ...
      buffer = (int*)rg_malloc(allocSize  * sizeof(int));
      if (buffer == NULL)
      {
         return INDEX_MEMORY_ERROR;
      }
      ...
      //================================//
      // rg_free the old data if necessary //
      //================================//
      if (index->indices != NULL)
      {
         rg_free(index->indices);
      }
                   ...
                   )
{
   ...
   *result = -1;
   ...
   while (low != high)
   {
	   ...
	   *result = compare(value, (void*)( reinterpret_cast<ptrdiff_t>(data) + indices[mid] * stride));
