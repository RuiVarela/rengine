...
extern void rg_free(void *p);
extern void* rg_realloc(void *p, size_t size);
...
#define FREE(p)   if (p) { rg_free(p); p = NULL; }
...
static void CalculateMeshNormals(mesh3ds_t* mesh)
{
	...
	//======================//
	mesh->norms = (float(*)[3])rg_malloc(sizeof(float) * 3 * mesh->triCount * 3);
	if (mesh->norms == NULL)
	{
		return;
	}
	...
	//============================================================================//
	triRefs = (long(*)[50])rg_malloc(sizeof(long) * 50 * vertIndex.count);
	if (triRefs == NULL)
	{
		return;
	}
	...
	//========================================//
	faceNorms = (float(*)[3])rg_malloc(sizeof(float) * 3 * mesh->triCount);
	...
	// build a table that links each vertex to all triangles that use said vertex //
	//============================================================================//
	triRefs = (long(*)[50])rg_malloc(sizeof(long) * 50 * mesh->vertCount);
	if (triRefs == NULL)
	{
	return;
	}
	...
	*/
	...
	//==========================================//
	// rg_free up the local buffers that were used //
	//==========================================//
	if (triRefs)
	{
		rg_free(triRefs);
...
static void SmoothMeshTangentSpace(mesh3ds_t* mesh)
{
   ...
   newTanSpace = (float(*)[9])rg_malloc(sizeof(float) * 9 * mesh->triCount * 3);
   if (newTanSpace == NULL)
   {
      return;
   }
   ...
   //===========================================================//
   tanRefs = (int(*)[50])rg_malloc(sizeof(int) * 50 * mesh->vertCount);
   if (tanRefs == NULL)
   {
      return;
   }
           ...
           (mesh->verts[mesh->tris[i][1]][2] == mesh->verts[mesh->tris[i][2]][2])))
      {
         ...
         for (j = 0; j < mesh->groupCount; j++)
         {
            ...
            if (found != NULL)
            {
               ...
               //TODO: GCC 4.4 upgrade, needs recheck
            	k = ptrdiff_t(found - mesh->groups[j].tris) / sizeof(ptrdiff_t);
...
static void CalculateMeshTangentSpace(mesh3ds_t* mesh)
{
   ...
   //========================//
   mesh->tangentSpace = (float(*)[9])rg_malloc(sizeof(float) * 9 * mesh->triCount * 3);
   if (mesh->tangentSpace == NULL)
   {
      return;
   }
...
//   long*          oldSmooth = NULL;
   ...
   {
      data->groups = (group3ds_t*)rg_malloc(sizeof(group3ds_t));
      if (data->groups)
      {
         data->groupCount = 1;
         ...
         data->groups[0].tris = (long*)rg_malloc(sizeof(long) * data->triCount);
   ...
   tris = (triangle_t*)rg_malloc(data->triCount * sizeof(triangle_t));
   temp = (triangle_t*)rg_malloc(data->triCount * sizeof(triangle_t));
...
long Read3dsFile(char const* pFilename, data3ds_t* output)
{
   ...
   memset(output, 0, sizeof(data3ds_t));
	...
	if (g_pFile)
	{
      ...
      size_t fread_size = fread(&chunk.id, sizeof(chunk.id), 1, g_pFile);
      fread_size = fread(&chunk.length, sizeof(chunk.length), 1, g_pFile);
...
void ReadString(char* string)
{
	...
	do
	{
		fread_size = fread(&string[i], sizeof(char), 1, g_pFile);
	}
	while (string[i++] != '\0');
}
...
void ReadMain3ds(chunk3ds_t* chunk, data3ds_t* output)
{
   ...
   switch (chunk->id)
	...
	case 0x3d3d: // Edit3ds
      ...
      if (output->meshCount)
      {
         output->meshes = (mesh3ds_t*)rg_malloc(sizeof(mesh3ds_t) * output->meshCount);
         memset(output->meshes, 0, sizeof(mesh3ds_t) * output->meshCount);
      }

      if (output->materialCount)
      {
         output->materials = (material3ds_t*)rg_malloc(sizeof(material3ds_t) * output->materialCount);
...
void ReadVertexList(mesh3ds_t* output)
{
	...
	size_t fread_size = fread(&shNumCoords, sizeof(unsigned short), 1, g_pFile);

	output->vertCount = shNumCoords;
	output->verts = (float(*)[3]) rg_malloc(sizeof(float) * 3 * output->vertCount);

	fread_size = fread(output->verts, sizeof(float), 3 * output->vertCount, g_pFile);
}
...
void ReadFaceList(mesh3ds_t* output, long length)
{
	...
	size_t fread_size = fread(&shNumFaces, sizeof(unsigned short), 1, g_pFile);
	pos += sizeof(unsigned short);

	output->triCount = shNumFaces;
	output->tris = (long(*)[3]) rg_malloc(sizeof(long) * 3 * output->triCount);

	unsigned short verts[4];

	for (i = 0; i < shNumFaces; i++)
	{
		fread_size = fread(verts, sizeof(short), 4, g_pFile);
		for (int j = 0; j < 3; j++)
		{
   ...
   if (pos < length)
   {
      ...
      {
         output->groups = (group3ds_t*)rg_malloc(sizeof(group3ds_t) * output->groupCount);
...
void ReadFaceSubs(chunk3ds_t* chunk, mesh3ds_t* output)
{
	...
	switch (chunk->id)
	{
		...
		output->smooth = (long*) rg_malloc(sizeof(long) * output->triCount);
		if (output->smooth)
		{
			fread_size = fread(output->smooth, sizeof(long), output->triCount, g_pFile);
		}
			break;
	};
}
...
void ReadTextureCoordinates(mesh3ds_t* output)
{
	...
	output->texCoords = (float(*)[2])rg_malloc(sizeof(float) * 2 * output->texCoordCount);
	fread_size = fread(output->texCoords, sizeof(float), 2 * output->texCoordCount, g_pFile);
}

void ReadLocalAxis(mesh3ds_t* output)
{
	size_t fread_size = fread(output->axis, sizeof(float), 9, g_pFile);
	fread_size = fread(output->position, sizeof(float), 3, g_pFile);
}
...
void ReadMaterialGroup(group3ds_t* group)
{
	...
	size_t fread_size = fread(&numFaces, sizeof(unsigned short), 1, g_pFile);

	group->tris = (long*)rg_malloc(numFaces * sizeof(long));
	...
	if (group->tris)
	{
		...
		{
			fread_size = fread(&face, sizeof(unsigned short), 1, g_pFile);
			group->tris[i] = face;
		}
	}
}
...
void ReadColorChunk(chunk3ds_t* chunk, float* fColor)
{
	...
	{
		case 0x0010: // COLOR_F
		fread_size = fread(fColor, sizeof(float), 3, g_pFile);
			break;
		case 0x0011: // COLOR_24
		fread_size = fread(chRGB, sizeof(char), 3, g_pFile);
...
void ReadPercentageChunk(chunk3ds_t* chunk, float* value)
{
	...
	switch (chunk->id)
	{
		...
		fread_size = fread(&shPercent, sizeof(short), 1, g_pFile);
		*value = (float) shPercent;
			break;
		case 0x0031: // FLOAT_PERCENTAGE
		fread_size = fread(value, sizeof(float), 1, g_pFile);
			break;
		default:
			break;
	};
