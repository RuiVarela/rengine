...
AL_API ALvoid AL_APIENTRY alGenBuffers(ALsizei n,ALuint *puiBuffers)
{
    ...
    if (n > 0)
    {
        ...
        if (!IsBadWritePtr((void*)puiBuffers, n * sizeof(ALuint)))
        {
            ...
            while(i < n)
            {
                *list = rg_calloc(1, sizeof(ALbuffer));
                if(!(*list))
                {
                    while(end->next)
                    {
        ...
        if ((ALBuf->refcount==0)&&(data))
        {
            switch(format)
            {
                ...
                case AL_FORMAT_REAR32: {
                    ...
                    temp = rg_realloc(ALBuf->data, (BUFFER_PADDING*NewChannels + size) * NewBytes);
                    if(temp)
                    {
                        ALBuf->data = temp;
                ...
                case AL_FORMAT_STEREO_IMA4: {
                    ...
                    temp = rg_realloc(ALBuf->data, (BUFFER_PADDING*OrigChans + size)*NewBytes);
                    if(temp)
                    {
                        ALBuf->data = temp;
                ...
                case AL_FORMAT_71CHN_MULAW: {
                    ...
                    // Allocate extra padding samples
                    temp = rg_realloc(ALBuf->data, (BUFFER_PADDING*Channels + size)*NewBytes);
                    if(temp)
                    {
                        ALBuf->data = temp;
