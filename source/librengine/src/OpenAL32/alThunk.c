...
void alThunkInit(void)
{
    ...
    g_ThunkArraySize = 1;
    g_ThunkArray = rg_calloc(1, g_ThunkArraySize * sizeof(ThunkEntry));
}

void alThunkExit(void)
{
    rg_free(g_ThunkArray);
    g_ThunkArray = NULL;
    g_ThunkArraySize = 0;
...
ALuint alThunkAddEntry(ALvoid *ptr)
{
    ...
    if(index == g_ThunkArraySize)
    {
        ...
        NewList = rg_realloc(g_ThunkArray, g_ThunkArraySize*2 * sizeof(ThunkEntry));
        if(!NewList)
        {
            LeaveCriticalSection(&g_ThunkLock);
            AL_PRINT("rg_realloc failed to increase to %u enties!\n", g_ThunkArraySize*2);
            return 0;
        }
